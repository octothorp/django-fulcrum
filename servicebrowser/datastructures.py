...
class EasyModel(object):
    ...
    def api_json_url(self):
        return mark_safe('%s%s/%s.json' % (self.site.root_url, self.model._meta.app_label, self.model._meta.module_name))
        
    def api_xml_url(self): 
        return mark_safe('%s%s/%s.xml' % (self.site.root_url, self.model._meta.app_label, self.model._meta.module_name))
    
    def api_poxml_url(self): 
        return mark_safe('%s%s/%s.poxml' % (self.site.root_url, self.model._meta.app_label, self.model._meta.module_name))
    ...
    """
    Returns Queryset Outside of Easy Model
    """
    def unformatted_query_set(self):
        qs = self.model._default_manager.get_query_set()._clone()
        return qs
...
class EasyInstance(object):
    ...
    def api_json_url(self):
        return mark_safe('%s%s/%s/objects/%s.json' % (self.model.site.root_url, self.model.model._meta.app_label, self.model.model._meta.module_name, iri_to_uri(self.pk())))
        #return mark_safe('/api/%s/%s.json' % (self.model.model._meta.app_label, iri_to_uri(self.pk())))

    def api_xml_url(self):
        return mark_safe('%s%s/%s/objects/%s.xml' % (self.model.site.root_url, self.model.model._meta.app_label, self.model.model._meta.module_name, iri_to_uri(self.pk())))
        #return mark_safe('/api/%s/%s.xml' % ( self.model.model._meta.app_label, iri_to_uri(self.pk())))

    def api_poxml_url(self):
        return mark_safe('%s%s/%s/objects/%s.poxml' % (self.model.site.root_url, self.model.model._meta.app_label, self.model.model._meta.module_name, iri_to_uri(self.pk())))
    ...
    def exposed_fields(self):
        ...
        for f in self.model.model._meta.fields + self.model.model._meta.many_to_many:
            logging.debug( pprint( self.model.model_databrowse().expose_fields ) )
            for field in self.model.model_databrowse().expose_fields:
                if( f.name == field ):
                    yield EasyInstanceField( self.model, self, f )
    ...
    def related_objects(self):
        ...
        for rel_object in self.model.model._meta.get_all_related_objects() + self.model.model._meta.get_all_related_many_to_many_objects():
            ...
            # -- Skip if Model References Self, generates some error in the code
            if self.model.verbose_name == em.verbose_name:
                continue
