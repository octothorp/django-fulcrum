...
class FieldChoicePlugin(DatabrowsePlugin):
    ...
    def model_view(self, request, model_databrowse, url, extension):
        ...
        print( "extension: " + unicode( extension ) )
        ...
        if self.fields.has_key(url_bits[0]):
            return self.field_view( request, extension, self.fields[url_bits[0]], *url_bits[1:] )
    ...
    def homepage_view(self, request):
        ...
        return render_to_response('servicebrowser/fieldchoice_homepage.html', {'root_url': self.site.root_url, 'model': easy_model, 'field_list': field_list})

    # -- Field/Value Specific View
    def field_view( self, request, extension, field, value = None ):
        easy_model = EasyModel(self.site, self.model)
        ...
        if value is not None:
            ...
            if extension == "json":
                data = serializers.serialize( "json", self.model.objects.all().filter( **{field.name: value} ) )
                return http.HttpResponse( data, "application/json" )
            elif extension == "xml":
                data = serializers.serialize( "xml", self.model.objects.all().filter( **{field.name: value} ) )
                return http.HttpResponse( data, "text/xml" )
            else:
                obj_list = easy_model.objects(**{field.name: value})
                return render_to_response('servicebrowser/fieldchoice_detail.html', {'root_url': self.site.root_url, 'model': easy_model, 'field': easy_field, 'value': value, 'object_list': obj_list})

        # -- Field List View
        if extension == "json":
            data = serializers.serialize( "json", self.model._default_manager.distinct().order_by(field.name) )
            return http.HttpResponse( data, "application/json" )
        elif extension == "xml":
            data = serializers.serialize( "xml", self.model._default_manager.distinct().order_by(field.name) )
            return http.HttpResponse( data, "text/xml" )
        else:
            obj_list = [v[field.name] for v in self.model._default_manager.distinct().order_by(field.name).values(field.name)]
            return render_to_response('servicebrowser/fieldchoice_list.html', {'root_url': self.site.root_url, 'model': easy_model, 'field': easy_field, 'object_list': obj_list})
